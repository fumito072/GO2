...
## 次 task(Exclusive Actuation Gateway)へ送る項目

- gateway の受入条件: (a) `release_latch` は認証済み operator lease 経由のみ、
  (b) `Directive.stop_state` は latch 種別であり stop FSM の target ではない
  (物理進行は stop_transitions の表に従う)、(c) ACTIVE_HOLD 中に STOP_NOW
  latch を受けたら ACTIVE_HOLD 滞在を維持し未定義遷移を評価しない
- **policy_hash と run manifest backend の突合**(Branch L run での
  ASCEND/DESCEND + not_applicable 拒否 — 契約層は形式のみ検査)
- STOP_NOW 拒否時エスカレーションの enforcement
- S/L 排他・mode transition handshake(request → ack inactive → generation → enable)
