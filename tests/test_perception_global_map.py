...
class TestSafetySemantics(unittest.TestCase):
    ...
    def test_optimistic_mask_counts_unknown_but_not_occupied(self):
        # 探索計画専用の optimistic モード(操作者要望 2026-07-17):
        # UNKNOWN は通行可に含めるが、OCCUPIED とその inflate は従来どおり不可
        m = make_map()
        m.integrate_scan((0.0, 0.0), [(2.0, 0.0)], now_ns=1_000)
...
class TestEvidenceDecay(unittest.TestCase):
    ...
    def test_clear_footprint_purges_even_hazard(self):
        # ロボットが立っている円盤は最強の通行可能証拠 — hazard も解除。
        # 半径外の実在の壁には触れない
        m = make_map()
        ...
        wall = m.world_to_cell(2.0, 0.0)
        self.assertEqual(m.grid[near[1], near[0]], FREE)
        self.assertEqual(m.occ_score[near[1], near[0]], 0)
        self.assertEqual(m.grid[wall[1], wall[0]], OCCUPIED)
