...
invariant(docs/CLAUDE.md §4):
  ...
  9. unknown ≠ free。pose/点群の途絶は fail-closed(新規移動を発行しない)。
     実行はMissionAgentのExplorationController + CollisionGuardへ一本化し、
     本クラスはproposal/confirm/STOP/UI表示だけを担当する。

開始条件: ARM 中のみ + 確認済み GoalSpec のみ。自動再開なし。

自動登坂は別GoalSpec・別確認・authority handoffが実装されるまで無効。
"""
import json
import math
import os
...
from cockpit.mission import (
    autonomy_robot_state_error, autonomy_sensor_error,
    capture_autonomy_sensors,
)
...
STAIR_AUTO = False          # 別GoalSpec/確認/authority handoff未実装のためfail-closed
...
class ExploreTask:
    ...
    def __init__(self, bridge, artifacts_dir: str = None, stair_task=None,
        ...
        self._ev_seq = 0
        self._events = []
        self._delegated_goal_id = None
        self._saved_map_id = None
        # 地図の唯一のwriterはMissionAgent/ExplorationController。本facadeは
        # map_frame/snapshotでcopyを読むだけにする。
        self._z_floor = None
        self._map_lock = threading.RLock()

    def _active_map(self):
        if self._mission is not None:
            return getattr(self._mission, "gmap", None)
        return self.gmap

    def _owns_delegate(self):
        spec = getattr(self._mission, "goal_spec", None) if self._mission else None
        return (self._delegated_goal_id is not None and spec is not None and
                spec.goal_id == self._delegated_goal_id)
    ...
    def _stage_proposal(self, prop: GoalProposal) -> dict:
        if self._run or (self._mission is not None and
                         getattr(self._mission, "_run_flag", False)):
            ...
            if wp is None:
                return {"kind": "rejected", "handled": True,
                        "say": "ウェイポイント %r が未登録です(先に探索を完了してください)"
                               % prop.target.ref}
            return {"kind": "rejected", "handled": True,
                    "say": "ウェイポイント走行は安全controller統合前のため無効です"}
    ...
    def confirm(self) -> dict:
        ...
        if self._mission is None:
            self.status = "refused"
            self.detail = "安全探索runnerが接続されていません"
            return {"kind": "error", "handled": True,
                    "say": "開始拒否: " + self.detail}
        err = self._mission.start_goal(spec, execu)
        if err:
            self.status, self.detail = "refused", err
            deploy_log("explore_refused", reason=err)
            return {"kind": "error", "handled": True, "say": "開始拒否: " + err}

        self._run = True
        self._delegated_goal_id = spec.goal_id
        self._saved_map_id = None
        self.status = "running"
        self.detail = "開始"
        self.say = "開始します"
    ...
    def _operator_stop(self, stop_spec: GoalSpec):
        ...
        self._run = False
        if self._mission is not None:
            self._mission.abort("STOP_NOW(操作者)")
    ...
    def abort(self, why: str = "user"):
        ...
        if self._owns_delegate() and self._mission is not None:
            self._mission.abort("explore中断: " + why)
    ...
    def _sync_delegate_state(self):
        """MissionAgentの単一runner状態を既存EXPLORE UI形式へ反映する。"""
        if self._mission is None or not self._owns_delegate():
            return
        mstatus = getattr(self._mission, "status", "idle")
        if mstatus == "running":
            self._run = True
            self.status = "running"
            self.detail = getattr(self._mission, "detail", "探索中")
            self.say = self.detail
            goal = (getattr(self._mission, "last", {}) or {}).get("goal")
            self.goal = tuple(goal) if goal else None
            return
        if not self._run and self.status in ("done", "aborted", "error", "stopped"):
            return
        self._run = False
        self.goal = None
        self.status = {"done": "done", "error": "error",
                       "aborted": "aborted"}.get(mstatus, "stopped")
        self.detail = getattr(self._mission, "detail", mstatus)
        self.say = self.detail
        gm = self._active_map()
        if gm is not None and self._saved_map_id != gm.map_id:
            self._save_map(completed=self.status == "done")
            self._saved_map_id = gm.map_id

    def snapshot(self) -> dict:
        self._sync_delegate_state()
        g = self._active_map()
        map_lock = (getattr(self._mission, "map_lock", None)
                    if self._mission is not None else self._map_lock)
        if g is not None and map_lock is not None:
            with map_lock:
                counts = g.counts()
                waypoints = sorted(g.waypoints)
        else:
            counts = g.counts() if g is not None else None
            waypoints = sorted(g.waypoints) if g is not None else []
        trace = self.trace
        controller = getattr(self._mission, "controller", None) \
            if self._mission is not None else None
        if controller is not None:
            trace = list(controller.explorer.robot_trace)
        with self._lock:
            pend = self._pending
    ...
    def map_frame(self):
        ...
        if map_lock is not None:
            with map_lock:
                g = gm.grid.copy()
                origin = tuple(gm.origin_xy)
                resolution = float(gm.resolution_m)
        else:
            g = gm.grid.copy()
            origin = tuple(gm.origin_xy)
            resolution = float(gm.resolution_m)
        # UI送信は約0.1m/cell。mapのnative resolutionに応じて安全側max-pool。
        factor = max(1, int(round(0.10 / resolution)))
        h2, w2 = g.shape[0] // factor, g.shape[1] // factor
        g4 = g[:h2 * factor, :w2 * factor].reshape(
            h2, factor, w2, factor).max(axis=(1, 3))
        head = struct.pack("<BfffHH", BIN_EXPMAP,
                           float(origin[0]), float(origin[1]),
                           float(resolution * factor), w2, h2)
    ...
    def _health_check(self) -> str:
        ...
        state_error = autonomy_robot_state_error(self.bridge)
        if state_error:
            return state_error
        # callback受信時刻ではなく、処理に成功した同一世代のpose/cloudを検査。
        # wrong frame/parse失敗でrx時刻だけ更新されても古い点群を再利用しない。
        return autonomy_sensor_error(capture_autonomy_sensors(self.bridge)) or ""
    ...
    def _loop_inner(self, spec: GoalSpec, execu: MissionExecutive):
        raise RuntimeError(
            "legacy ExploreTask actuator runner is disabled; use MissionAgent.start_goal")
        ...
        try:
            ...
            d["provenance"] = {"run_id": "cockpit_explore",
                               ...
                               "saved_at": time.strftime("%Y-%m-%dT%H:%M:%SZ",
                                                         time.gmtime())}
            path = os.path.join(self._artifacts, gm.map_id)
            os.makedirs(path, exist_ok=True)
            with open(os.path.join(path, "map.json"), "w",
                      encoding="utf-8") as f:
                json.dump(d, f, ensure_ascii=False)
            deploy_log("explore_map_saved", path=path, completed=completed,
                       counts={"unknown": sum(row.count(0) for row in d["cells"]),
                               "free": sum(row.count(1) for row in d["cells"]),
                               "occupied": sum(row.count(2) for row in d["cells"])})
        except Exception as e:
