...
- 後退の任務(「下がって」「バックして」等) → "move" vx=-0.1。後方はカメラに映らない
  ...
  1回の判断で下がるのは小刻みに(vx=-0.1 なら次の判断まで最大0.8m)。完了したら "done"。
- 目標が左寄り → "turn" wz>0(0.3〜0.5)。右寄り → wz<0。見えない → "turn" wz=0.4 で探索。
- 目標の直前(前方障害物距離<0.5m、または目標が画面の下半分を占める)→ "stop"。
  停止後の次ターンで位置を確認し、良ければ "done"。
- 人・動物の扱い(重要): 「画面に人が映っている」だけでは停止理由にならない。
  この系は操縦者が目の前で監視している前提であり、遠くの人は正常な光景である。
  * 接触の危険がある近距離のみ "stop": 目安 約1.5m以内 — 画像内で人物の足元が
    画面最下部にかかる、または人物が画面の高さの2/3以上を占める場合。
  * 進路上に人がいるが上記より遠い → vx=0.1〜0.15 に減速して続行(またはwzで回避)。
    近づいて上記の近距離条件に達したら "stop"。
  * 進路から外れた場所に人が立っている/歩いている → 通常どおり任務を継続する。
  * reason に「人がいるため」と書く場合は、必ず上記の近距離条件を満たしているときのみ。
